...
SELECT * FROM W_PRODUCT_D T WHERE T.PROD_NUM='100578782';
SELECT * FROM W_PRODUCT_ATTR_D T WHERE T.PROD_NUM='100578782';
...
INSERT INTO W_PRODUCT_DS
  (PRODUCT_TYPE_CODE,
   ...
   BBG_ITEM_BRAND,
   BBG_ITEM_UOM_DESC)
  SELECT ITEM.ITEM_NUMBER_TYPE,
         ITEM.STANDARD_UOM,
         DATE '2013-12-09',
         1 /*DATASOURCE_NUM_ID*/,
         ITEM.ITEM,
         DIVISION.DIVISION || '~' || GROUPS.GROUP_NO || '~' || ITEM.DEPT || '~' ||
         ITEM.CLASS || '~' || ITEM.SUBCLASS, /*PROD_CAT5*/
         ITEM.PACK_IND, /*PACK_FLG*/
         DIVISION.DIVISION || '~' || GROUPS.GROUP_NO || '~' || ITEM.DEPT || '~' ||
         ITEM.CLASS || '~' || ITEM.SUBCLASS, /*PROD_CAT5_AS_WAS*/
         ITEM.ITEM /*PROD_NUM*/,
   ...
   WHERE GROUPS.DIVISION = DIVISION.DIVISION
     ...
     AND ITEM.ITEM IN
         (SELECT T.PROD_NUM
           ...
           WHERE T.SCD1_WID = A.SCD1_WID
             AND A.PRODUCT_ATTR12_NAME = A.PRODUCT_ATTR11_NAME
             AND T.PROD_NUM NOT IN
                 (SELECT T.INTEGRATION_ID FROM W_PRODUCT_DS_TL T));
