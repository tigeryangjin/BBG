...
as
SELECT ORG.ORG_NUM,ORG_NAME.ORG_NAME,DH.LVL4ANC_PRODCAT_ID,D2.C1,PROD_NUM.PROD_NUM,PROD_NAME.PRODUCT_NAME,
NOWINV.INV_SOH_QTY,NOWINV.INV_SOH_COST_AMT_LCL,
...
YJ_SLS_IT_LC_MN_V SLS,
     W_INT_ORG_D ORG,
		 W_PROD_CAT_DH DH inner join (select T16687.DOMAIN_MEMBER_NAME as c1,
                         ...
                         ) D2 On concat(concat(concat(concat(concat(concat('SBC', '~'), DH.LVL6ANC_PRODCAT_ID), '~'), DH.LVL5ANC_PRODCAT_ID), '~'), DH.LVL4ANC_PRODCAT_ID) = D2.c2 and DH.DATASOURCE_NUM_ID = D2.c3,
		 --W_INT_ORG_DH ORGDH,
...
AS
SELECT T.ORG_WID,T.PROD_WID,SUM(T.SLS_QTY-T.RET_QTY) SLS_QTY,SUM((T.SLS_AMT_LCL-T.RET_AMT_LCL)-(T.SLS_TAX_AMT_LCL-T.RET_TAX_AMT_LCL)) SLS_AMT
FROM W_RTL_SLS_IT_LC_DY_A T
WHERE T.DT_WID BETWEEN '120130801000' AND '120130831000'
GROUP BY T.ORG_WID,T.PROD_WID;
--*********
SELECT ORG.ORG_NUM,ORG_NAME.ORG_NAME,DH.LVL4ANC_PRODCAT_ID,D2.C1,PROD_NUM.PROD_NUM,PROD_NAME.PRODUCT_NAME,
SLS.SLS_QTY,SLS.SLS_AMT
FROM 
--INV
...
;
-------------------------------------------------------------------------------------------------------
SELECT sls_amt,DECODE(SLS_AMT,0,'zero',sls_amt) FROM YJ_SLS_IT_LC_MN_V;
SELECT * FROM W_PROD_CAT_DH;
SELECT * FROM W_RTL_SLS_IT_LC_DY_A;
SELECT T.ORG_WID,T.PROD_WID,T.DT_WID,T.INV_SOH_QTY,T.INV_SOH_COST_AMT_LCL 
  FROM W_RTL_INV_IT_LC_DY_FV T 
 WHERE T.DT_WID IN ('120130831000','120130930000');
--商品编码
SELECT * FROM W_PRODUCT_D_TL;
SELECT * FROM W_PRODUCT_D;
SELECT * FROM W_PRODUCT_ATTR_D;
(SELECT   T.*
 FROM   W_PRODUCT_D T,
        W_PRODUCT_ATTR_D A
 WHERE   T.SCD1_WID =
