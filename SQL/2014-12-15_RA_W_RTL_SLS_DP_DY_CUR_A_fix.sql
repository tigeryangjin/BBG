--2014.09月份W_RTL_SLS_DP_DY_CUR_A数据有问题,需要修复
SELECT T.BBG_RETAIL_TYPE_WID,
       SUM((T.SLS_AMT_LCL - T.RET_AMT_LCL) -
           (T.SLS_TAX_AMT_LCL - T.RET_TAX_AMT_LCL))
  FROM RADM.W_RTL_SLS_DP_LC_MN_A T
 WHERE T.MN_WID = 201409
   AND T.PROD_DH_WID = 233
   --AND T.BBG_RETAIL_TYPE_WID IN (1, 29, 30, 60006)
 GROUP BY T.BBG_RETAIL_TYPE_WID;

SELECT T.BBG_RETAIL_TYPE_WID,
       SUM((T.SLS_AMT_LCL - T.RET_AMT_LCL) -
           (T.SLS_TAX_AMT_LCL - T.RET_TAX_AMT_LCL))
  FROM RADM.W_RTL_SLS_DP_LC_DY_A T
 WHERE T.DT_WID BETWEEN 120140901000 AND 120140930000
   AND T.PROD_DH_WID = 233
   --AND T.BBG_RETAIL_TYPE_WID IN (1, 29, 30, 60006)
 GROUP BY T.BBG_RETAIL_TYPE_WID;
...
SELECT * FROM RADM.W_PROD_CAT_DH T WHERE T.LEVEL_NAME = 'DEPT';
