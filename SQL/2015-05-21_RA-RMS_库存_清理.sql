...
SELECT A.ITEM,
 ...
 WHERE A.ITEM = B.PROD_IT_NUM
   AND A.LOC = B.ORG_NUM
   AND A.SUPPLIER = B.SUPPLIER_NUM
   AND NVL(A.SOH, 0) <> NVL(B.SUPP_INV_QTY, 0);
--FIX BBG_RA_SUPP_INV_IT_LC_G
...
--BBG_RA_SUPP_INV_IT_LC_G重复处理
SELECT T.ORG_NUM, T.PROD_IT_NUM, T.SUPPLIER_NUM, COUNT(*)
  FROM RADM.BBG_RA_SUPP_INV_IT_LC_G T
 GROUP BY T.ORG_NUM, T.PROD_IT_NUM, T.SUPPLIER_NUM
HAVING COUNT(*) > 1;
--UPDATE BBG_RA_SUPP_INV_IT_LC_DY_F

--DELETE BBG_RA_SUPP_INV_IT_LC_G
DELETE RADM.BBG_RA_SUPP_INV_IT_LC_G A
 WHERE EXISTS
 (SELECT 1
          FROM (SELECT C.ROW_WID
                  FROM (SELECT D.ORG_NUM,
                               D.PROD_IT_NUM,
                               D.SUPPLIER_NUM,
                               MIN(D.ROW_WID) ROW_WID
                          FROM RADM.BBG_RA_SUPP_INV_IT_LC_G D
                         WHERE EXISTS
                         (SELECT 1
                                  FROM (SELECT T.ORG_NUM,
                                         ...
                                         GROUP BY T.ORG_NUM,
                                                  ...
                                                  T.SUPPLIER_NUM
                                        HAVING COUNT(*) > 1) E
                                 WHERE D.ORG_NUM = E.ORG_NUM
                                   AND D.PROD_IT_NUM = E.PROD_IT_NUM
                                   AND D.SUPPLIER_NUM = E.SUPPLIER_NUM)
                         GROUP BY D.ORG_NUM, D.PROD_IT_NUM, D.SUPPLIER_NUM) C) B
...
--3,RMS,RA都有
DROP TABLE RADM.JIN_INV_20150528_TMP;
CREATE TABLE RADM.JIN_INV_20150528_TMP AS
  SELECT B.ITEM,
         B.LOC,
...
CREATE UNIQUE INDEX PK_JIN_INV_20150528_TMP ON JIN_INV_20150528_TMP(ITEM,
                                                                    LOC);

--FIX W_RTL_INV_IT_LC_G
--删除W_RTL_INV_IT_LC_G中,同一item_loc的早一条记录
DELETE RADM.W_RTL_INV_IT_LC_G D
 WHERE EXISTS
 (SELECT 1
          FROM (SELECT A.PROD_IT_NUM, A.ORG_NUM, MIN(A.ROW_WID) ROW_WID
                  FROM RADM.W_RTL_INV_IT_LC_G A
                 WHERE EXISTS (SELECT 1
                          FROM (SELECT C.PROD_IT_NUM, C.ORG_NUM
                                  FROM RADM.W_RTL_INV_IT_LC_G C
                                 GROUP BY C.PROD_IT_NUM, C.ORG_NUM
                                HAVING COUNT(*) > 1) B
                         WHERE A.PROD_IT_NUM = B.PROD_IT_NUM
                           AND A.ORG_NUM = B.ORG_NUM)
                 GROUP BY A.PROD_IT_NUM, A.ORG_NUM) E
         WHERE D.ROW_WID = E.ROW_WID);
...
UPDATE RADM.W_RTL_INV_IT_LC_G A
 ...
 WHERE A.INV_SOH_QTY <> 0
   AND EXISTS (SELECT 1
          FROM RADM.JIN_INV_20150528_TMP C
...
UPDATE RADM.W_RTL_INV_IT_LC_DY_F A
 ...
 WHERE A.TO_DT_WID = 999999999999999
   AND A.INV_SOH_QTY <> 0
   AND EXISTS (SELECT 1
          FROM RADM.JIN_INV_20150528_TMP B,
               RADM.W_PRODUCT_D          P,
               RADM.W_INT_ORG_D          O
         WHERE B.ITEM = P.PROD_NUM
           AND B.LOC = O.ORG_NUM
           AND A.PROD_WID = P.ROW_WID
           AND A.ORG_WID = O.ROW_WID);
...
--***********************************************************************************
--ITEM INV
SELECT SUM(T.STOCK_ON_HAND) STOCK_ON_HAND,
       SUM(T.STOCK_ON_HAND * T.AV_COST) AV_COST,
       SUM(T.STOCK_ON_HAND * T.UNIT_COST) UNIT_COST
  FROM ITEM_LOC_SOH_HIST@RA_RMS_DBLINK T
 WHERE T.SOH_BAK_DATE = TRUNC(SYSDATE - 1);
...
--SUPP INV
SELECT SUM(T.SOH) SOH, SUM(T.TOTAL_COST) TOTAL_COST
