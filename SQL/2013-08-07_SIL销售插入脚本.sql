...
select     W_INT_ORG_D_RTL_TMP.ORG_WID,
  ...
  W_EMPLOYEE_D.ROW_WID,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.SLS_QTY,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.SLS_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.SLS_PROFIT_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.SLS_MANUAL_MKDN_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.SLS_MANUAL_MKUP_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.SLS_TAX_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.SLS_EMP_DISC_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.SLS_MANUAL_COUNT,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.SLS_SCAN_COUNT,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RET_QTY,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RET_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RET_PROFIT_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RET_MANUAL_MKDN_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RET_MANUAL_MKUP_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RET_TAX_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RET_EMP_DISC_AMT_LCL,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RET_MANUAL_COUNT,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RET_SCAN_COUNT,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DOC_CURR_CODE,
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_CURR_CODE,
  (CASE
WHEN 
  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_CURR_CODE = STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DOC_CURR_CODE THEN 
    1.0
ELSE     
      (
SELECT  EX_RT.EXCH_RATE
FROM  RADM.W_EXCH_RATE_G   EX_RT
WHERE  EX_RT.W_FROM_CURCY_CODE = STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DOC_CURR_CODE
...
END ),
  (CASE
WHEN 
  W_GLOBAL_CURR_G.GLOBAL1_CURR_CODE IS NULL THEN 
  NULL
ELSE 
  (CASE
WHEN 
    W_GLOBAL_CURR_G.GLOBAL1_CURR_CODE = STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DOC_CURR_CODE THEN 
    1.0
ELSE 
    (CASE
WHEN 
      W_GLOBAL_CURR_G.GLOBAL1_CURR_CODE = STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_CURR_CODE AND NOT  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_EXCHANGE_RATE IS NULL THEN 
...
AND  EX_RT.END_DT > 
        STG_W_RTL_SLS_TRX_IT_LC_DY_FS.EXCHANGE_DT
AND  EX_RT.RATE_TYPE = 
        W_GLOBAL_CURR_G.GLOBAL1_RATE_TYPE
AND  EX_RT.DATASOURCE_NUM_ID = 
        STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DATASOURCE_NUM_ID
...
END ),
  (CASE
WHEN 
  W_GLOBAL_CURR_G.GLOBAL2_CURR_CODE IS NULL THEN 
  NULL
ELSE 
  (CASE
WHEN 
    W_GLOBAL_CURR_G.GLOBAL2_CURR_CODE = STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DOC_CURR_CODE THEN 
    1.0
ELSE 
    (CASE
WHEN 
      W_GLOBAL_CURR_G.GLOBAL2_CURR_CODE = STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_CURR_CODE AND NOT  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_EXCHANGE_RATE IS NULL THEN 
      STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_EXCHANGE_RATE
ELSE 
      (
SELECT  EX_RT.EXCH_RATE
...
AND  EX_RT.END_DT > 
        STG_W_RTL_SLS_TRX_IT_LC_DY_FS.EXCHANGE_DT
AND  EX_RT.RATE_TYPE = 
        W_GLOBAL_CURR_G.GLOBAL2_RATE_TYPE
AND  EX_RT.DATASOURCE_NUM_ID = 
        STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DATASOURCE_NUM_ID
...
END ),
  (CASE
WHEN 
  W_GLOBAL_CURR_G.GLOBAL3_CURR_CODE IS NULL THEN 
  NULL
ELSE 
  (CASE
WHEN 
    W_GLOBAL_CURR_G.GLOBAL3_CURR_CODE = STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DOC_CURR_CODE THEN 
    1.0
ELSE 
    (CASE
WHEN 
      W_GLOBAL_CURR_G.GLOBAL3_CURR_CODE = STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_CURR_CODE AND NOT  STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_EXCHANGE_RATE IS NULL THEN 
      STG_W_RTL_SLS_TRX_IT_LC_DY_FS.LOC_EXCHANGE_RATE
ELSE 
      (
SELECT  EX_RT.EXCH_RATE
...
AND  EX_RT.END_DT > 
        STG_W_RTL_SLS_TRX_IT_LC_DY_FS.EXCHANGE_DT
AND  EX_RT.RATE_TYPE = 
        W_GLOBAL_CURR_G.GLOBAL3_RATE_TYPE
AND  EX_RT.DATASOURCE_NUM_ID = 
        STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DATASOURCE_NUM_ID
...
END ),
  ...
  W_EMPLOYEE_D.AUX1_CHANGED_ON_DT,
  W_EMPLOYEE_D.AUX2_CHANGED_ON_DT,
  W_EMPLOYEE_D.AUX3_CHANGED_ON_DT,
  W_EMPLOYEE_D.AUX4_CHANGED_ON_DT,
  ...
  nvl(STG_W_RTL_SLS_TRX_IT_LC_DY_FS.BBG_RETAIL_TYPE_ID,27),
...
from  RADM.W_GLOBAL_CURR_G   W_GLOBAL_CURR_G, 
      ...
      RADM.W_RTL_SLS_TRX_IT_LC_DY_FS   STG_W_RTL_SLS_TRX_IT_LC_DY_FS
where  (1=1)
 And ((((W_MCAL_DAY_D.MCAL_DAY_DT BETWEEN W_MCAL_WEEK_D.MCAL_WEEK_START_DT AND W_MCAL_WEEK_D.MCAL_WEEK_END_DT)) AND W_MCAL_DAY_D.DATASOURCE_NUM_ID=W_MCAL_WEEK_D.DATASOURCE_NUM_ID))
AND ((W_EMPLOYEE_D.EMPLOYEE_NUM=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.EMPLOYEE_NUM) AND W_EMPLOYEE_D.DATASOURCE_NUM_ID=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DATASOURCE_NUM_ID)
AND ((W_PRODUCT_D_RTL_TMP.PROD_IT_NUM=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.PROD_IT_NUM) AND W_PRODUCT_D_RTL_TMP.SRC_EFF_FROM_DT <= STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DAY_DT AND W_PRODUCT_D_RTL_TMP.SRC_EFF_TO_DT >= STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DAY_DT)
AND ((W_MCAL_DAY_D.DATASOURCE_NUM_ID=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DATASOURCE_NUM_ID) AND W_MCAL_DAY_D.MCAL_DAY_DT=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DAY_DT)
AND (W_XACT_TYPE_D.DATASOURCE_NUM_ID=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DATASOURCE_NUM_ID AND W_XACT_TYPE_D.W_XACT_CAT_CODE = 'RETAIL_TRAN_TYPE' and W_XACT_TYPE_D.W_XACT_TYPE_CODE = STG_W_RTL_SLS_TRX_IT_LC_DY_FS.RTL_TYPE_CODE)
AND ((W_INT_ORG_DH_RTL_TMP.ORG_NUM=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.ORG_NUM) AND W_INT_ORG_DH_RTL_TMP.EFFECTIVE_FROM_DT<=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DAY_DT AND W_INT_ORG_DH_RTL_TMP.EFFECTIVE_TO_DT>=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DAY_DT)
AND (BBG_RA_RETAIL_TYPE_D.RETAIL_TYPE_ID=nvl(STG_W_RTL_SLS_TRX_IT_LC_DY_FS.BBG_RETAIL_TYPE_ID,27))
AND (BBG_RA_ITEM_LOC_D.ITEM=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.PROD_IT_NUM AND BBG_RA_ITEM_LOC_D.LOC=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.ORG_NUM)
AND ((W_INT_ORG_D_RTL_TMP.ORG_NUM=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.ORG_NUM) AND W_INT_ORG_D_RTL_TMP.EFFECTIVE_FROM_DT<=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DAY_DT AND W_INT_ORG_D_RTL_TMP.EFFECTIVE_TO_DT>=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DAY_DT)
AND (STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DATASOURCE_NUM_ID=W_GLOBAL_CURR_G.DATASOURCE_NUM_ID)
AND (W_MCAL_DAY_D.MCAL_CAL_WID=W_MCAL_CONTEXT_G.MCAL_CAL_WID)
AND (STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DATASOURCE_NUM_ID=W_MCAL_CONTEXT_G.DATASOURCE_NUM_ID)
AND (W_MCAL_CONTEXT_G.MCAL_CAL_WID=W_MCAL_WEEK_D.MCAL_CAL_WID)
AND (((LPAD(W_MINUTE_OF_DAY_D.HOUR_24_NUM,2,'0')||LPAD(W_MINUTE_OF_DAY_D.MINUTE_NUM,2,'0'))=LPAD(STG_W_RTL_SLS_TRX_IT_LC_DY_FS.MIN_NUM,4,'0')) AND W_MINUTE_OF_DAY_D.DATASOURCE_NUM_ID=STG_W_RTL_SLS_TRX_IT_LC_DY_FS.DATASOURCE_NUM_ID)
--And ((W_MCAL_CONTEXT_G.ORG_ID = '#RA_BI.RA_ORG_ID') AND W_MCAL_CONTEXT_G.CALENDAR_ID = '#RA_BI.RA_CALENDAR_ID')
 --And (STG_W_RTL_SLS_TRX_IT_LC_DY_FS.ETL_THREAD_VAL = #RA_BI.RA_THREAD_VAL)
