...
--检查漏掉的数据的商品地点维度
SELECT *
  FROM RADM.BBG_RA_SLS_TRX_REF_V T
 WHERE T.day_dt = DATE '2013-04-30'
   AND NOT EXISTS (SELECT 1
          FROM W_RTL_SLS_TRX_IT_LC_DY_F      F,
               W_INT_ORG_D                   D,
               RABATCHER.W_PRODUCT_D_RTL_TMP P
         WHERE F.ORG_WID = D.ROW_WID
           AND F.PROD_WID = P.PROD_IT_WID
           AND T.prod_it_num = P.PROD_IT_NUM
