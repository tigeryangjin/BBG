...
--0.数据准备
--**********************************************************************************
--0.1.CREATE TABLE BBG_ORDER_VIP_TMP AS
SELECT H.ORDERID,
       H.BUYERMOBILE,
...
--0.1.1 Add comments to the columns 
comment on column BBG_ORDER_VIP_TMP.orderid is '订单号';
comment on column BBG_ORDER_VIP_TMP.buyermobile is '手机号码';
comment on column BBG_ORDER_VIP_TMP.order_date is '订单日期';
comment on column BBG_ORDER_VIP_TMP.ordertotalmoney is '订单总金额';
comment on column BBG_ORDER_VIP_TMP.order_qty is '订单总数量';
comment on column BBG_ORDER_VIP_TMP.isvip is '是否步步高会员';
...
--0.3.取消订单导入
CREATE TABLE JDDJ_ORDER_TMP(订单编号 VARCHAR2(20),
                            订单总金额 NUMBER(20, 4),
                            用户应付金额 NUMBER(20, 4),
                            门店应收金额 NUMBER(20, 4),
                            门店收单时间 DATE,
...
SELECT * FROM RADM.JDDJ_ORDER_TMP FOR UPDATE;

--0.4.siebel会员资料导入
CREATE TABLE BBG_SIEBEL_JDDJ_VIP(XM姓名 VARCHAR2(100),
                                 SFZH身份证号码 VARCHAR2(100),
                                 TEL手机号 VARCHAR2(50),
                                 KH会员卡号 VARCHAR2(64),
                                 CSRQ出生日期 DATE,
                                 XB性别 VARCHAR2(4),
...
SELECT * FROM RADM.BBG_SIEBEL_JDDJ_VIP FOR UPDATE;

--更新BBG_SIEBEL_JDDJ_VIP会员卡号
UPDATE RADM.BBG_SIEBEL_JDDJ_VIP T
   SET T.KH会员卡号 =
       (SELECT S.CARD_NUM
          FROM RADM.BBG_JDDJ_VIP_BIRTHDAY S
         WHERE T.TEL手机号 = S.PHONE_NUMBER)
 WHERE EXISTS (SELECT 1
          FROM RADM.BBG_JDDJ_VIP_BIRTHDAY S
         WHERE T.TEL手机号 = S.PHONE_NUMBER
           AND T.KH会员卡号 <> S.CARD_NUM);

SELECT *
  FROM RADM.BBG_JDDJ_VIP_BIRTHDAY T
 WHERE T.PHONE_NUMBER = 18670320919;
SELECT * FROM RADM.BBG_SIEBEL_JDDJ_VIP T WHERE T.TEL手机号 = 18670320919;
  ...
  FROM (SELECT T.HYK_NO,
         ...
         WHERE T.HYK_NO = V.CARD_NUM
           ...
           AND V.CARD_NUM NOT IN ('006066135352',
                                  ...
                                  '006064860368',
                                  '030020053794')
         GROUP BY T.HYK_NO, SUBSTR(T.DT_WID, 2, 6), V.AGE_ZONE) A
  ...
  FROM (SELECT T.HYK_NO,
         ...
         WHERE T.HYK_NO = V.CARD_NUM
           ...
           AND V.CARD_NUM NOT IN ('006066135352',
                                  ...
                                  '006064860368',
                                  '030020053794')
         GROUP BY T.HYK_NO, V.AGE_ZONE) A
...
--******************************************************************************
--5.1渠道分析
SELECT T.MD门店 注册地, COUNT(1) 整体会员人数
  FROM RADM.BBG_SIEBEL_JDDJ_VIP T
 WHERE EXISTS (SELECT 1
          FROM RADM.BBG_JDDJ_VIP_BIRTHDAY S
         WHERE T.KH会员卡号 = S.CARD_NUM)
 GROUP BY T.MD门店
 ORDER BY 2 DESC;

--5.2线下会员渠道分析
SELECT T.MD门店 注册地, COUNT(1) 参与线下消费会员人数
  FROM RADM.BBG_SIEBEL_JDDJ_VIP T
 WHERE EXISTS (SELECT /*+PARALLEL(16)*/
         1
          FROM RADM.BBG_RA_RFM_VIP_F S
         WHERE S.DT_WID BETWEEN 120140101000 AND 120160920000
           AND T.KH会员卡号 = S.HYK_NO)
   AND EXISTS (SELECT 1
          FROM RADM.BBG_JDDJ_VIP_BIRTHDAY U
         WHERE T.KH会员卡号 = U.CARD_NUM)
   AND T.KH会员卡号 NOT IN ('006066135352',
                        '006064858772',
                        '006061488200',
                        '006064860097',
                        '006062619094',
...
--5.3没来线下消费
SELECT T.MD门店 注册地, COUNT(1) 没来线下消费会员人数
  FROM RADM.BBG_SIEBEL_JDDJ_VIP T
...
--5.4线下会员渠道分析(按月)
SELECT /*+PARALLEL(16)*/
 SUBSTR(S.DT_WID, 2, 6) 月份,
 T.MD门店 注册地,
 COUNT(DISTINCT T.KH会员卡号) 参与线下消费会员人数
  FROM RADM.BBG_SIEBEL_JDDJ_VIP T, RADM.BBG_RA_RFM_VIP_F S
 WHERE T.KH会员卡号 = S.HYK_NO
   AND S.DT_WID BETWEEN 120140101000 AND 120160920000
   AND EXISTS (SELECT 1
          FROM RADM.BBG_JDDJ_VIP_BIRTHDAY U
         WHERE T.KH会员卡号 = U.CARD_NUM)
   AND T.KH会员卡号 NOT IN ('006066135352',
                        ...
                        '006064860368',
                        '030020053794')
 GROUP BY SUBSTR(S.DT_WID, 2, 6), T.MD门店
 ORDER BY 1, 2;
