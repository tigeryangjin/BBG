...
SELECT SUM(A.ORDERTOTALMONEY) 线上总销售额,
 ...
 WHERE A.ORDER_DATE BETWEEN DATE '2016-08-23' AND DATE '2016-09-11';
...
SELECT SUM(A.ORDERTOTALMONEY) 线上会员销售额,
  ...
  FROM BBG_ORDER_VIP_TMP A
 WHERE A.ORDER_DATE BETWEEN DATE '2016-08-23' AND DATE '2016-09-20'
   AND A.ISVIP = 1;
...
--2.1.2年龄 
SELECT TRUNC(ROUND((DATE '2016-09-11' - T.BIRTHDAY) / 365), -1) 年龄段,
       COUNT(T.PHONE_NUMBER) 人数,
       RATIO_TO_REPORT(COUNT(T.PHONE_NUMBER)) OVER() 占比
  FROM BBG_JDDJ_VIP_BIRTHDAY T
 GROUP BY TRUNC(ROUND((DATE '2016-09-11' - T.BIRTHDAY) / 365), -1)
 ORDER BY 1;

--2.2线下购物频次
SELECT /*+PARALLEL(16)*/
 COUNT(DISTINCT A.HYK_NO) 会员人数,
 COUNT(A.SLS_TRX_ID) 交易笔数,
 SUM(A.SLS_AMT_LCL - A.RET_AMT_LCL) 交易金额
...
SELECT A.MONTH 月份,
  ...
  FROM (SELECT /*+PARALLEL(16)*/
         ...
         TRUNC(ROUND((DATE '2016-09-11' - V.BIRTHDAY) / 365), -1) AGE_GROUP,
         ...
         GROUP BY T.HYK_NO, SUBSTR(T.DT_WID, 2, 6), V.BIRTHDAY) A
...
SELECT A.AGE_GROUP 年龄段, COUNT(A.HYK_NO) 会员人数
  FROM (SELECT /*+PARALLEL(16)*/
         ...
         WHERE T.HYK_NO = V.CARD_NUM
           AND T.DT_WID BETWEEN 120150901000 AND 120160911000
         GROUP BY T.HYK_NO, V.BIRTHDAY) A
 GROUP BY A.MONTH, A.AGE_GROUP
 ORDER BY 1, 2;
