...
--******************************************************************************
--2.2.1线上会员线下购物人数
SELECT COUNT(V.CARD_NUM) 会员线下购物人数
  FROM RADM.BBG_JDDJ_VIP_BIRTHDAY V
 WHERE EXISTS (SELECT 1
          FROM RADM.BBG_RA_RFM_VIP_F R
         WHERE V.CARD_NUM = R.HYK_NO
           AND R.DT_WID BETWEEN 120140101000 AND 120160920000);

SELECT COUNT(DISTINCT A.HYK_NO), SUM(A.SLS_AMT_LCL - A.RET_AMT_LCL)
...
SELECT TRUNC(V.AGE, -1) 年龄段 COUNT(V.CARD_NUM) 会员线下购物人数
  FROM RADM.BBG_JDDJ_VIP_BIRTHDAY V
 WHERE EXISTS (SELECT 1
          FROM RADM.BBG_RA_RFM_VIP_F R
         WHERE V.CARD_NUM = R.HYK_NO
...
--2.2.3线下购物频次
SELECT COUNT(DISTINCT C.HYK_NO) 会员人数,
       COUNT(C.SLS_TRX_ID) 交易笔数,
       SUM(C.SALES_AMT) 交易金额
  FROM (SELECT /*+PARALLEL(16)*/
         A.HYK_NO, A.SLS_TRX_ID, A.SLS_AMT_LCL - A.RET_AMT_LCL SALES_AMT
          FROM RADM.BBG_RA_RFM_VIP_F A
         WHERE EXISTS
         (SELECT /*+PARALLEL(16)*/
                 1
