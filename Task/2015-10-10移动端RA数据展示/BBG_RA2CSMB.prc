CREATE OR REPLACE PROCEDURE BBG_RA2CSMB AS
BEGIN
  DECLARE
    CURSOR CUR_DAYS IS
      SELECT TO_DATE(I.MCAL_NUM, 'YYYYMMDD') DAYS, I.MCAL_WID DTWID
        FROM RADM.W_RTL_CURR_MCAL_G I
       WHERE I.MCAL_TYPE = 'DT';
  ...
  BEGIN
    ...
    WHILE CUR_DAYS%FOUND LOOP
              ...
              A.TOSLS MONTH_NET_SALES_AMT --月累计净销售额
               FROM (SELECT /*+PARALLEL(16)*/
                      ...
                      WHERE MCAL.ROW_WID = CUST.DT_WID
                        ...
                        AND TRUNC(MCAL.MCAL_DAY_DT) = V_DAYS
               ...
               FROM RADM.W_MCAL_DAY_DV MCAL,
                    (SELECT /*+PARALLEL(16)*/
                      ...
                      SUM(C.VIP_BBG_CUSTOMER_COUNT) VIP_CUST
                       FROM RADM.W_RTL_SLS_LC_DY_A   S,
                            RADM.BBG_RA_CUST_LC_DY_A C
                      WHERE S.DT_WID = V_DTWID
      ...
      COMMIT;
      --MAR05
      /*MERGE INTO MAR05@CSMBDB T
      USING (SELECT TRUNC(MCAL.MCAL_DAY_DT) DAYS, --日期
                    O.ORG_HIER11_NUM REGION, --区域
                    T.AMT net_sales_amt, --净销售额
                    T.CUST CUST, --来客数
                    DECODE(T.CUST, 0, 0, T.AMT / T.CUST) PCT, --客单
                    DECODE(T.AMT, 0, 0, T.PROFIT / T.AMT) GP, --毛利率
                    T.VIP_AMT vip_net_sales_amt, --会员净销售额
                    T.VIP_CUST VIP_CUST, --会员来客数
                    DECODE(T.VIP_CUST, 0, 0, T.VIP_AMT / T.VIP_CUST) vip_pct --会员客单
               FROM RADM.W_MCAL_DAY_DV MCAL,
                    (SELECT S.DT_WID,
                            S.ORG_DH_WID,
                            SUM((S.SLS_AMT_LCL - S.SLS_TAX_AMT_LCL) -
              ...
              WHERE MCAL.ROW_WID = T.DT_WID
                AND T.ORG_DH_WID = O.ROW_WID
              ORDER BY O.ORG_HIER11_NUM) S
      ON (T.DAYS = S.DAYS AND T.REGION = S.REGION)
      WHEN MATCHED THEN
        UPDATE
           SET T.net_sales_amt     = S.net_sales_amt,
               T.CUST              = S.CUST,
