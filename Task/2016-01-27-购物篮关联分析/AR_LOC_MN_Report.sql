...
SELECT MBA.MN_WID,
       ...
       MBA.AB_CNT,
       MBA.LOC_CNT,
       MBA.AB_CNT / MBA.LOC_CNT AB_SUPPORT,
       MBA.AB_CNT / MBA.A_CNT A_CONFIDENCE,
       MBA.AB_CNT / MBA.B_CNT B_CONFIDENCE,
       (MBA.LOC_CNT * MBA.AB_CNT) / (MBA.A_CNT * MBA.B_CNT) UPGRADES
                  ...
                  FROM --购物篮AB关联
                       (SELECT DISTINCT SUBSTR(A.DT_WID, 2, 6) MN_WID,
                         ...
                         WHERE A.SLS_TRX_ID = B.SLS_TRX_ID
                           ...
                           AND A.DT_WID BETWEEN &BEGIN_DT_WID AND &END_DT_WID
                           AND EXISTS
                         (SELECT 1
                                  FROM RADM.W_INT_ORG_D O
