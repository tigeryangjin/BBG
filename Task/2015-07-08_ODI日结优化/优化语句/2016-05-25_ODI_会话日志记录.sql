...
SELECT SE.SESS_NO,
       ...
       TS.NB_ERR,
			 TS.COL_TXT,
			 TS.DEF_TXT
  FROM RADM.SNP_SESSION       SE,
       RADM.SNP_SESS_STEP     ST,
       RADM.SNP_SESS_TASK_LOG TS
 WHERE SE.SESS_NO = ST.SESS_NO
   AND SE.SESS_NO = TS.SESS_NO
   AND ST.NNO = TS.NNO
   AND ST.NB_RUN = TS.NB_RUN
      --AND SE.SESS_NO = 616049001
   AND ST.STEP_NAME = 'INSERT_BBG_JL_ITEM_LOC_DAY '
   AND TS.TASK_NAME3 = 'INSERT INTO RA_RMS.BBG_JL_ITEM_LOC_DAY'
...
SELECT T.SESS_NO,
       ...
       T.NB_DEL,
       T.NB_ERR
  FROM RADM.SNP_SESSION T
 WHERE T.SESS_NAME = 'SIL_ITEMDIMENSION'
 ORDER BY T.SESS_BEG DESC;
...
SELECT TRUNC(SE.SESS_BEG) DATES,
       ...
       TS.TASK_DUR,
       TS.NB_UPD,
       DECODE(TS.TASK_DUR, 0, 0, TS.NB_UPD / TS.TASK_DUR) UPD_PER_SECOND, --每秒更新数
