...
SELECT * FROM RAINY_USER_MERCH T WHERE T.USER_ID='R8002144277';
SELECT * FROM RAINY_USER_MERCH T WHERE T.USER_ID='R7805225025';
SELECT * FROM RAINY_USER_MERCH T WHERE T.USER_ID='R8706172065';
SELECT * FROM RAINY_USER_MERCH T WHERE T.USER_ID='R7606133826';
...
where  RU.USER_ID IN ('R8002144277' ,'R7805225025' ,'R8706172065','R7606133826' );

SELECT * FROM RAINY_USER_MERCH T WHERE T.USER_ID='R7310263053';

SELECT * FROM RAINY_USER_MERCH T WHERE T.USER_ID='R8110046939';

SELECT * FROM RAINY_USER_LOC T WHERE T.USER_ID='R7310263053';

SELECT * FROM RAINY_USER_LOC T WHERE T.USER_ID='R8110046939';

SELECT * FROM STORE T WHERE T.STORE_NAME LIKE'%交通店%';
120017	湘天桥
120154	茶陵交通店
...
--注意下面条件，当用户地点权限为ALL时才是=，不是ALL就是<>
   ...
   AND RU.USER_ID in ('R7310263053' ,'R8110046939')
...
'uniquemember: cn='||a.USER_ID||',cn=users,dc=bbg' ||CHR (10 )
FROM RAINY_USER_MERCH a left join rainy_user ru on a.user_id=ru.user_id
where  RU.USER_ID IN ('R7310263053' ,'R8110046939');
--用户地点权限导入
--查询出来的结果保存成txt文本文件，并且要把【”】去掉，然后用LDAP导入，file-->import 
--SIM,RA/BIP系统的用户的创建、权限的修改，都是通过LDAP导入。
SELECT DISTINCT
...
SELECT * FROM RAINY_USER T WHERE T.USER_ID='R661117402x';
...
SELECT * FROM user_attrib T WHERE T.USER_ID='R661117402x';
DELETE user_attrib WHERE USER_ID='R661117402x';
