insert into rainy_user(user_id,user_name,id) values('R8604043928','马进','2002');
insert into rainy_user(user_id,user_name,id) values('R8002032518','余中定','2003');

--权限变更的用户修改ID列，从BBG_Security_Application_User_List表中获取
SELECT * FROM RAINY_USER
WHERE USER_ID IN ( 'R8604043928','R8002032518') FOR UPDATE;
...
SELECT 'revoke ' || rr.role_ext || ' from ' || ru.user_id || ';'
 ...
 WHERE rr.appl = 'RMS'
   ...
   AND RU.ID BETWEEN 2002 AND 2003;
...
where  RU.ID BETWEEN 2002 AND 2003 ;
...
DELETE FROM RSM_USER_ROLE T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2002 AND 2003);
DELETE FROM SEC_USER_GROUP T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2002 AND 2003);
DELETE FROM RAINY_USER_LOC T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2002 AND 2003);
DELETE FROM RAINY_USER_MERCH T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2002 AND 2003);
DELETE FROM RAINY_USER_POSITION T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2002 AND 2003);

insert into rainy_user_position(user_id,position_id) values('R8604043928',58);
insert into rainy_user_position(user_id,position_id) values('R8002032518',58);
-----------------------------
insert into rainy_user_merch(user_id,merch) values('R8604043928','99');
insert into rainy_user_merch(user_id,merch) values('R8002032518','99');
---------------------------------
...
from rainy_user ru
where  to_number (RU.id ) BETWEEN 2002 AND 2003
