insert into rainy_user(user_id,user_name,id) values('R7205222536','钟少青','2878');
insert into rainy_user(user_id,user_name,id) values('R7108070038','孙书桥','2879');
insert into rainy_user(user_id,user_name,id) values('R8812280036','姚亮','2880');

insert into rainy_user(user_id,user_name,id) values('R7510010274','王志方','2884');
insert into rainy_user(user_id,user_name,id) values('R8410060958','孙在杰','2885');
insert into rainy_user(user_id,user_name,id) values('R8307083018','唐若捷','2886');
...
to ' || ru.user_id || ' ;' || chr ( 13 ) ||
  ...
  FROM rainy_user ru
  WHERE RU.ID BETWEEN 2878 AND 2889 ;

--权限变更的用户修改ID列，从BBG_Security_Application_User_List表中获取
SELECT * FROM RAINY_USER
WHERE USER_ID in ( 'R8111295732','R790917003X','R7308210342','R7909131055','R8706172065') FOR UPDATE;
--insert into rainy_user(user_id,user_name,id) values('R8111295732','卿海建','2881');
--insert into rainy_user(user_id,user_name,id) values('R790917003X','韦林','2882');
--insert into rainy_user(user_id,user_name,id) values('R7308210342','黄晓红','2883');
--insert into rainy_user(user_id,user_name,id) values('R7909131055','朱博','2887');
--insert into rainy_user(user_id,user_name,id) values('R8706172065','李虹','2889');
...
SELECT 'revoke ' || rr.role_ext || ' from ' || ru.user_id || ';'
 ...
 WHERE rr.appl = 'RMS'
   ...
   AND RU.ID BETWEEN 2878 AND 2889;
...
DELETE FROM RSM_USER_ROLE T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2878 AND 2889);
DELETE FROM SEC_USER_GROUP T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2878 AND 2889);
DELETE FROM RAINY_USER_LOC T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2878 AND 2889);
DELETE FROM RAINY_USER_MERCH T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2878 AND 2889);
DELETE FROM RAINY_USER_POSITION T WHERE EXISTS(SELECT 1 FROM RAINY_USER RU WHERE T.USER_ID=RU.USER_ID AND RU.ID BETWEEN 2878 AND 2889);
...
from rainy_user ru
where  to_number (RU.id ) BETWEEN 2878 AND 2889
