...
SELECT * FROM DBA_ROLE_PRIVS T WHERE T.granted_role IN ('STPRCCB','STPRCCA') AND EXISTS(SELECT 1 FROM RAINY_USER_POSITION RUP WHERE T.GRANTEE=RUP.USER_ID AND RUP.POSITION_ID IN (84))
AND EXISTS(SELECT 1 FROM RAINY_USER_LOC RUL WHERE RUL.USER_ID=T.GRANTEE AND RUL.LOC='120033');
