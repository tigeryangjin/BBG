--用户表
insert into rainy_user(user_id,user_name,id) values('R7909090012','杨进','905');

--职位
insert into rainy_user_position(user_id,position_id) values('R7909090012',36);

--类别
insert into rainy_user_merch(user_id,merch) values('R7909090012','ALL');
																																							
--店别
insert into rainy_user_loc (user_id,loc)values('R7909090012','ALL');
...
to ' || rs.user_id || ' ;' || chr (13 ) ||
  ...
  FROM rainy_user rs 
  WHERE rs.user_id IN('R7909090012');
...
SELECT 'insert into user_attrib(user_id,user_name,lang) values(' || chr( 39) ||
  ...
  FROM rainy_user ru 
  WHERE RU.USER_ID IN ('R7909090012');
  
insert into user_attrib(user_id,user_name,lang) values('R7909090012','杨进',8);
...
SELECT * FROM RAINY_USER WHERE USER_ID IN ('R7909090012');
SELECT * FROM user_attrib WHERE USER_ID IN ('R7909090012');
...
SELECT * FROM RAINY_USER WHERE to_number(id) between 905 and 905 ;
